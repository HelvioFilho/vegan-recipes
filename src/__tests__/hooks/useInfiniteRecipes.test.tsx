...
import { RecipesResponseProps, useInfiniteRecipes } from '@/hooks/useInfiniteRecipes';
...
describe('useInfiniteRecipes Hook', () => {
  ...
  it('must successfully search for recipes (page 1) and have next page', async () => {
    mockedApi.get.mockResolvedValueOnce({ data: mockSuccessPage1 });

    const { result, unmount } = renderHook(() => useInfiniteRecipes(), {
      wrapper: Wrapper,
    });
  ...
  it('should use default pageParam=1 in the queryFn', async () => {
    ...
    expect(mockedApi.get).toHaveBeenCalledWith('/recipes?page=1');
    unmount();
  });

  it('should call api.get with pageParam 2 when fetching next page', async () => {
    mockedApi.get
    ...
    expect(mockedApi.get).toHaveBeenCalledWith('/recipes?page=2');
